...
function artifactStat(file) {
    const stat = fs.lstatSync(file);
    if (stat.isSymbolicLink() || (!stat.isFile() && !stat.isDirectory())) {
        throw new Error('Unsupported artifact (links and special files are not allowed): ' + file);
    }
...
function fileHash(file) {
    if (!artifactStat(file).isFile()) throw new Error('Expected a regular file: ' + file);
    return crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex');
}

// Freeze the complete copy plan before creating a destination. Case-only names
// remain distinct on Linux, but must still have compatible types and contents.
function mergeArtifacts(srcDirs) {
    const entries = new Map();
    const folded = new Map();
    function visit(dir, relative) {
        const src = path.join(dir, relative);
        const stat = artifactStat(src);
        const entry = { src, relative, directory: stat.isDirectory(),
            hash: stat.isFile() ? fileHash(src) : null };
        const key = relative.toLowerCase();
        const previous = folded.get(key);
        if (previous && (previous.directory !== entry.directory ||
            (!entry.directory && previous.hash !== entry.hash))) {
            throw new Error('Conflicting merge artifact: ' + relative + ' (' +
                previous.src + ' and ' + src + ')');
        }
        folded.set(key, entry);
        if (!entries.has(relative)) entries.set(relative, entry);
        if (entry.directory) {
            for (const name of fs.readdirSync(src)) {
                const child = path.join(relative, name);
                if (child === path.join('rewind-snapshots', 'index.json') ||
                    child === path.join('checkpoints', 'index.md')) continue;
                if (child.toLowerCase() === path.join('checkpoints', 'index.md')) {
                    throw new Error('Unsupported case-only checkpoint index path: ' + child);
                }
                visit(dir, child);
            }
        }
    ...
    for (const dir of srcDirs) {
        for (const sub of ['files', 'research', 'rewind-snapshots', 'checkpoints']) {
            const src = path.join(dir, sub);
            if (!optionalStat(src)) continue;
            if (!artifactStat(src).isDirectory()) throw new Error('Expected artifact directory: ' + src);
            visit(dir, sub);
        }
    }
...
function copyArtifacts(entries, newDir) {
    for (const entry of entries) {
        const dest = path.join(newDir, entry.relative);
        if (entry.directory) fs.mkdirSync(dest, { recursive: true });
        else fs.copyFileSync(entry.src, dest, fs.constants.COPYFILE_EXCL);
    }
}

function verifyArtifacts(entries, newDir) {
    for (const entry of entries) {
        const dest = path.join(newDir, entry.relative);
        const stat = artifactStat(dest);
        if (stat.isDirectory() !== entry.directory ||
            (!entry.directory && fileHash(dest) !== entry.hash)) {
            throw new Error('Merged artifact verification failed: ' + entry.relative);
        }
    }
}

// removeRecursive — recursive removal compatible with Node 12 (no fs.rmSync).
...
function repairEvents(lines) {
    ...
    const parsed = parseResult.parsed.filter(ev => {
        if (ev.json.id === '' ||
            (ev.type === 'tool.execution_complete' && ev.json.data && ev.json.data.model === 'unknown')) {
            droppedInvalid++;
            return false;
        }
        return ev.type !== 'session.error' && ev.type !== 'session.warning';
    });
    ...
    return { lines: output, droppedMalformed: parseResult.droppedMalformed + droppedInvalid };
}
...
function checkpointBody(dir, reference) {
    const parts = reference.split(/[\\/]/);
    if (path.isAbsolute(reference) || reference.includes(':') ||
        parts.some(p => p === '' || p === '.' || p === '..' || /[. ]$/.test(p)) ||
        /^\[.*\]\(.*\)$/.test(reference) || /[`<>]/.test(reference) ||
        (parts.length === 1 && parts[0].toLowerCase() === 'index.md')) {
        throw new Error('Unsupported checkpoint body reference: ' + reference);
    }
    let file = path.join(dir, 'checkpoints');
    for (const part of parts) {
        if (!artifactStat(file).isDirectory()) {
            throw new Error('Checkpoint body parent is not a directory: ' + file);
        }
...
function readCheckpointRows(dir) {
    const file = path.join(dir, 'checkpoints', 'index.md');
    if (!optionalStat(file)) return [];
    fileHash(file);
    const lines = readLines(file);
    const rows = [];
    let recognized = false;
    for (const line of lines) {
        if (/^\|\s*#\s*\|\s*Title\s*\|\s*File\s*\|\s*$/i.test(line)) {
            recognized = true;
            continue;
        }
        if (/^\|\s*:?-+:?\s*\|\s*:?-+:?\s*\|\s*:?-+:?\s*\|\s*$/.test(line)) continue;
        if (!/^\s*\|/.test(line)) continue;
        const match = /^\|\s*\d+\s*\|[^|]*\|([^|]+)\|\s*$/.exec(line);
        if (!match || !match[1].trim()) {
            throw new Error('Unsupported checkpoint index row in ' + file +
                ': expected # | Title | File with a plain relative path.');
        }
        const reference = match[1].trim();
        checkpointBody(dir, reference);
        rows.push({ line, reference });
        recognized = true;
    }
    if (!recognized && lines.some(line => line.trim())) {
        throw new Error('Unsupported checkpoint index format: ' + file);
    }
    return rows;
}
...
function cmdMerge(newDir, srcDirs) {
    ...
    const artifacts = mergeArtifacts(srcDirs);
    const sessions = srcDirs.map(d => {
        ...
        return {
            ...
            checkpoints: readCheckpointRows(d),
        };
    });
    ...
    for (const s of sessions) {
        ...
        if (optionalStat(idxFile)) {
            fileHash(idxFile);
            snapshotIndexes.push(JSON.parse(fs.readFileSync(idxFile, 'utf8')));
        } else snapshotIndexes.push(null);
    }

    if (optionalStat(newDir)) {
        throw new Error('Merge destination already exists: ' + newDir);
    }
    fs.mkdirSync(newDir);
    ownedMergeDestination = newDir;
    ...
    for (const s of byUpdated) {
        checkpointRows.push(...s.checkpoints);
    }
    let n = 1;
    const renumbered = checkpointRows.map(r => r.line.replace(/^\|\s*\d+/, `| ${n++}`));
    const header = ['# Checkpoint History',
        ...
        '', '| # | Title | File |', '|---|-------|------|'];
    const checkpointIndex = path.join(newDir, 'checkpoints', 'index.md');
    const checkpointText = header.concat(renumbered).join('\n') + '\n';
    fs.writeFileSync(checkpointIndex, checkpointText, 'utf8');
    ...
    copyArtifacts(artifacts, newDir);
    ...
    verifyArtifacts(artifacts, newDir);
    if (!artifactStat(checkpointIndex).isFile() ||
        fs.readFileSync(checkpointIndex, 'utf8') !== checkpointText) {
        throw new Error('Merged checkpoint index failed read-back verification.');
    }
    for (const row of checkpointRows) checkpointBody(newDir, row.reference);
    ...
    ownedMergeDestination = null;
}

let ownedMergeDestination = null;
...
try {
    ...
    if (ownedMergeDestination !== null &&
        (fs.existsSync(ownedMergeDestination) || (() => {
            try { return fs.lstatSync(ownedMergeDestination).isSymbolicLink(); } catch (_) { return false; }
        })())) {
        try {
            removeRecursive(ownedMergeDestination);
        } catch (cleanupError) {
            process.stderr.write('Failed to remove partial merge destination: ' +
